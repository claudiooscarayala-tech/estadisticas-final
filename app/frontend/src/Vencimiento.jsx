...
function Vencimiento({ companyName }) {
  ...
  return (
    <div className="report-container">
      <div className="report-header">
        <h1 className="report-title">Gestión de Vencimientos {companyName}</h1>
      </div>
      
      <div className="card" style={{ maxWidth: "600px", margin: "2rem auto" }}>
        <h2 style={{ marginBottom: "1rem", color: "var(--text-main)" }}>Subir Archivo Excel</h2>
        <p style={{ marginBottom: "1.5rem", color: "var(--text-light)" }}>
          Sube el archivo Excel con los registros de vencimientos de pólizas de {companyName}. El sistema agrupará por productor y les enviará un correo automáticamente con su archivo correspondiente.
        </p>
        ...
        >
          ...
          <input 
            ...
            id={`file-upload-vencimiento-${companyName.replace(/\s+/g, '-')}`}
            style={{ display: "none" }}
          />
          <label 
            htmlFor={`file-upload-vencimiento-${companyName.replace(/\s+/g, '-')}`}
